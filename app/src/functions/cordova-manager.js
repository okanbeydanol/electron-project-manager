...
class CordovaManager extends ChildProcess {
    ...
    async getCordovaVersion(secondTime = false) {
        return new Promise((resolve) => {
            this.execCommand('cordova -v', async (event) => {
                if (event.error) {
                    if (event.message.includes('const [name, version] = nameMap.get(release)')) {
                        if (!secondTime) {
                            const fixed = await this.fixMacOsReleaseName();
                            if (!fixed.error) {
                                return await this.getCordovaVersion(true);
                            }
    ...
    async fixMacOsReleaseName(global = true) {
        return new Promise(async (resolve) => {
            if (this.system_type.mac) {
                ...
                if (!findOsExistRegexMatch) {
                    ...
                    if (getLastIndexRegexMatch && getLastIndexRegexMatch[0].split('[').length > 1) {
                        ...
                        if (addNewOsNameRegexMatch) {
                            ...
                            return resolve({ error: true, data: null, message: 'No data to be corrected!' });
                        }
                    } else {
                        return resolve({ error: true, data: null, message: 'No data to be corrected!' });
                    }
                ...
                return resolve({ error: true, data: null, message: 'Your system is not mac!' });
            }
        });
    }
}
